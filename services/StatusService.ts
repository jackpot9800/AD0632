...
class StatusService {
  ...
  async initialize() {
    ...
    if (this.isInitialized) {
      console.log('=== STATUS SERVICE ALREADY INITIALIZED v1.3.4 ===');
      return;
    }
    
    console.log('=== INITIALIZING STATUS SERVICE v1.3.4 ===');
    ...
    this.heartbeatInterval = setInterval(async () => {
      try {
        ...
        if (this.isInPresentationMode && (now - this.lastHeartbeatTime) < 45000) {
          console.log('=== SKIPPING HEARTBEAT - PRESENTATION MODE v1.3.4 ===');
          return;
        }
  ...
  private async sendHeartbeat() {
    try {
      ...
      console.log('=== SENDING HEARTBEAT v1.3.4 ===');
      ...
      if (response.ok) {
        console.log('Heartbeat sent successfully v1.3.4');
      } else {
        console.log('Heartbeat failed with status:', response.status);
      }
    } catch (error) {
      if (error.name === 'AbortError') {
        console.log('Heartbeat timeout - continuing without blocking v1.3.4');
      } else {
  ...
  private async checkForRemoteCommands() {
    try {
      ...
      if (error.name === 'AbortError') {
        console.log('Command check timeout - continuing v1.3.4');
      } else {
  ...
  private async executeRemoteCommand(command: RemoteCommand) {
    console.log('=== EXECUTING REMOTE COMMAND v1.3.4 ===', command);
  ...
  setPresentationMode(isActive: boolean) {
    console.log('=== SETTING PRESENTATION MODE v1.3.4 ===', isActive);
  ...
  updateStatus(updates: Partial<DeviceStatus>) {
    ...
    console.log('Status updated v1.3.4:', this.currentStatus);
