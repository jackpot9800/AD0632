...
class StatusService {
  ...
  async initialize() {
    ...
    if (this.isInitialized) {
      console.log('=== STATUS SERVICE ALREADY INITIALIZED v2.2.0 ===');
      return;
    }
    
    console.log('=== INITIALIZING STATUS SERVICE v2.2.0 ===');
    ...
    this.heartbeatInterval = setInterval(async () => {
      try {
        ...
        if (this.isInPresentationMode && (now - this.lastHeartbeatTime) < 45000) {
          console.log('=== SKIPPING HEARTBEAT - PRESENTATION MODE v2.2.0 ===');
          return;
        }
  ...
  private async sendHeartbeat() {
    try {
      ...
      console.log('=== SENDING HEARTBEAT v2.2.0 ===');
      ...
      if (response.ok) {
        console.log('Heartbeat sent successfully v2.2.0');
      } else {
        console.log('Heartbeat failed with status:', response.status);
      }
    } catch (error) {
      if (error.name === 'AbortError') {
        console.log('Heartbeat timeout - continuing without blocking v2.2.0');
      } else {
  ...
  private async checkForRemoteCommands() {
    try {
      ...
      if (error.name === 'AbortError') {
        console.log('Command check timeout - continuing v2.2.0');
      } else {
  ...
  private async executeRemoteCommand(command: RemoteCommand) {
    console.log('=== EXECUTING REMOTE COMMAND v2.2.0 ===', command);
  ...
  setPresentationMode(isActive: boolean) {
    console.log('=== SETTING PRESENTATION MODE v2.2.0 ===', isActive);
  ...
  updateStatus(updates: Partial<DeviceStatus>) {
    ...
    console.log('Status updated v2.2.0:', this.currentStatus);
